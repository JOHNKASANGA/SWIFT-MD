...
def classify_material_from_content_rules(text: str) -> Optional[dict]:
    """Classify unmistakable documents from their extracted content."""
    searchable = " ".join(text[:3000].lower().split())
    ...
    for category, phrases in rules:
        for phrase in phrases:
            position = searchable.find(phrase)

            if position == -1:
                continue

            evidence_start = text.lower().find(phrase)
            evidence = text[evidence_start:evidence_start + 180].strip()
...
def classify_material_content(material: dict, text: str) -> dict:
    rule_match = classify_material_from_content_rules(text)
